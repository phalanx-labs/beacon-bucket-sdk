...
func TestNormalUpload_Upload(t *testing.T) {
	// 创建客户端（连接到测试服务）
	client := NewClient(
		WithConnect("localhost"),
		WithPort("5589"),
		WithAppAccess("341596753619002368"),
		WithAppSecretKey("cs_e49d93365f7b4409bc051e698e17b8ae"),
	)

	// 测试数据 - 使用新的简洁 API
	resp, err := client.Upload(context.Background(), &types.UploadRequest{
...
func TestNewClient(t *testing.T) {
	...
	clientWithAuth := NewClient(
		WithConnect("localhost"),
		WithPort("5589"),
		WithAppAccess("test-access-id"),
		WithAppSecretKey("test-secret-key"),
	)
